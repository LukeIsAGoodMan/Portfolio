...
/* ═══════════════════════════════════════════════
   FinanceChatAgent — White-mode chat + mobile-adaptive panels
   Desktop: side-by-side (Chat | Side Panel)
   Mobile: Chat-only + mobile HUD bar + FAB drawers
   ═══════════════════════════════════════════════ */
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "sys-0",
      role: "system",
      text: "Simulation initialized. You are meeting with a high-net-worth client interested in an Equity-Linked Structured Note.",
    },
    {
      id: "client-0",
      role: "client",
    ...
    <>
      ...
      <div className="lg:hidden flex items-center justify-between px-4 py-2.5 mb-3 rounded-xl border border-white/[0.06] bg-[#0a0e17]">
        ...
        <div className="flex items-center gap-2.5">
          <motion.div
            className="w-7 h-7 rounded-full border border-cyan-400/20 flex items-center justify-center"
            style={{
              background: isProcessing
                ? "radial-gradient(circle at 35% 35%, rgba(34,211,238,0.3), transparent 60%)"
          ...
          >
            <svg viewBox="0 0 40 40" className="w-4 h-4">
              <ellipse cx="20" cy="20" rx="14" ry="6" fill="none"
                stroke={isProcessing ? "rgba(34,211,238,0.5)" : "rgba(148,163,184,0.2)"} strokeWidth="1" />
              <ellipse cx="20" cy="20" rx="6" ry="14" fill="none"
                stroke={isProcessing ? "rgba(34,211,238,0.4)" : "rgba(148,163,184,0.15)"} strokeWidth="1" />
            </svg>
          </motion.div>
        ...
        <div className="flex items-center gap-1.5">
          <span className={`font-semibold text-[16px] tracking-[-0.02em] ${scoreColor}`}>
            {complianceScore}
          </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4 w-full">
        ...
        <div className="rounded-2xl border border-[#e2e5ea] bg-white overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.06)] flex flex-col">
          {/* Header bar */}
          ...
          <AnimatePresence>
            {redFlag && (
              ...
              >
                <div className="px-4 md:px-5 py-3 bg-red-50 border-b border-red-100 flex items-start gap-3">
          ...
          <div
            ref={scrollRef}
            className="flex-1 min-h-[350px] max-h-[60vh] lg:h-[420px] lg:max-h-none overflow-y-auto px-4 md:px-5 py-5 space-y-4 scroll-smooth"
        ...
        {/* ── Side Panel (Desktop only — hidden on mobile, accessed via FAB) ── */}
        <div className="hidden lg:flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#0a0e17] p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-mono text-[11px] text-cyan-400/70 tracking-[0.02em]">GPT-5.4 mini LIVE</p>
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-6 right-4 flex flex-col gap-3 z-40" style={{ bottom: "calc(1.5rem + env(safe-area-inset-bottom, 0px))" }}>
        <button
          onClick={() => { setMentorOpen(true); setManualOpen(false); }}
          className="w-12 h-12 rounded-full bg-[#0a0e17] border border-white/[0.08] text-cyan-400 flex items-center justify-center shadow-[0_4px_16px_rgba(0,0,0,0.3)] active:scale-95 transition-transform"
        ...
        <button
          onClick={() => { setManualOpen(true); setMentorOpen(false); }}
          className="w-12 h-12 rounded-full bg-[#0a0e17] border border-white/[0.08] text-slate-400 flex items-center justify-center shadow-[0_4px_16px_rgba(0,0,0,0.3)] active:scale-95 transition-transform"
          aria-label="Reference Manual"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {mentorOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl border-t border-white/[0.08] bg-[#0a0e17] max-h-[70vh] overflow-y-auto"
            ...
            >
              ...
              <div className="px-5 pb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[9px] font-mono tracking-[0.04em] text-cyan-400/50 bg-cyan-400/[0.06] border border-cyan-400/10 rounded-full px-2.5 py-0.5">
                    {mentorSource}
                  </span>
                </div>
                ...
                <div className="space-y-1.5">
                  {scenario?.complianceRules.map((rule, i) => (
                    <p key={i} className="text-[11px] text-slate-500 leading-[1.5]">
                      {rule}
                    </p>
                  ))}
                </div>
      ...
      {/* ── Mobile Drawer: Compliance Manual ── */}
      <AnimatePresence>
        {manualOpen && (
          <>
            <motion.div
